...
const CopyrightNote = lazy(() =>
  import("../../UI/CopyrightNote/CopyrightNote")
);
...
export const BlogNote = (props) => {
  ...
  //image name if different from id
  ...
  const identifyBlogNote = () => {
    ...
    for (let note of ALL_BLOG_NOTES) {
      ...
      if (
        baseUrl + note.id === props.location.pathname ||
        baseUrl + note.id + "/" === props.location.pathname
      ) {
        //set note data
        ...
        setImageName(note.imageName); // Image name if different from id
  ...
  useEffect(() => {
    ...
    if (noteId.length > 0) {
      ...
      if (imageName) {
        importImage(imageName); //If the image name is different from the id, import this image
      } else {
        importImage(noteId); //Otherwise import an image named as the id
      }
    }
    ...
    noteId.length > 1 && (
      <BlogWrapper>
        ...
        <ThemeWrapper theme={noteCategory}>
          ...
          <AnimatedContent pose={visible ? "visible" : "hidden"}>
            <BlogNoteCredits
              title={noteTitle}
        ...
        <Suspense fallback={<SmallSpinner />}>
          <BlogNoteList linklist={relatedNotes} showCategories={true} />
          <AnimatedContent pose={visible ? "visible" : "hidden"}>
            <SubpageLinks lang={"en"} reloadPage={reloadPage} blog={true} />
            <SectionSeparator />
            <CopyrightNote />
          </AnimatedContent>
        </Suspense>
